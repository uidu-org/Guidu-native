import type { ImageSourcePropType, TextStyle, TouchableOpacityProps, ViewStyle } from 'react-native'
...
export interface IListProps
  extends Pick<RecyclerListViewProps, 'onEndReached' | 'onEndReachedThreshold' | 'onScroll'> {
...
export interface ISwipeableBubble extends IChatBubble, Pick<IChatty, 'onReply'> {
  children?: JSX.Element
}
...
export interface IFooterProps extends Pick<IChatty, 'replyingTo'> {
  ...
  onPressSend: (data: { text: string; repliedTo?: IMessage; media?: IMedia[] }) => void
...
export interface IScrollToBottomProps extends Pick<TouchableOpacityProps, 'onPress'> {
  containerStyle?: ViewStyle
  content?: JSX.Element
}
